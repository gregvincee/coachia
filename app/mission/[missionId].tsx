  ...
  return (
    <ScreenContainer className="p-0" containerClassName="bg-[#07080C]">
      ...
      <ScrollView contentContainerStyle={styles.content}>
        <View className="flex-row items-center border-b border-[#343947] px-5 pb-4 pt-3"><TouchableOpacity accessibilityRole="button" accessibilityLabel="Retour aux missions" onPress={() => router.back()} activeOpacity={0.75}><Text className="text-base font-semibold text-[#E5DFD2]">‹ Missions</Text></TouchableOpacity><Text className="ml-4 flex-1 text-right text-xs font-bold tracking-widest text-[#E8C98A]">{mission.label.toUpperCase()}</Text></View>
        <View className="gap-5 px-5 pt-6">
          <View><Text className="text-3xl font-bold leading-10 text-[#F5F1E8]">{mission.title}</Text><Text className="mt-3 text-base leading-6 text-[#AEB4C0]">{mission.goal}</Text></View>
          <View className="rounded-2xl border border-[#C89D56] bg-[#151820] p-4"><Text className="text-xs font-bold tracking-widest text-[#E8C98A]">RÉSULTAT À CONSERVER</Text><Text className="mt-2 text-base font-semibold leading-6 text-[#F5F1E8]">{mission.outcome}</Text></View>
          <View className="flex-row gap-3"><TouchableOpacity accessibilityRole="button" accessibilityState={{ expanded: showWhy }} onPress={() => setShowWhy((value) => !value)} activeOpacity={0.75} className="flex-1 rounded-xl border border-[#343947] bg-[#151820] px-3 py-3"><Text className="text-center font-semibold text-[#E5DFD2]">Pourquoi ?</Text></TouchableOpacity><TouchableOpacity accessibilityRole="button" accessibilityState={{ expanded: showExample }} onPress={() => setShowExample((value) => !value)} activeOpacity={0.75} className="flex-1 rounded-xl border border-[#343947] bg-[#151820] px-3 py-3"><Text className="text-center font-semibold text-[#E5DFD2]">Montre-moi</Text></TouchableOpacity></View>
          {showWhy ? <View className="rounded-2xl bg-[#1D2029] p-4"><Text className="text-sm leading-6 text-[#E5DFD2]">{mission.whyItMatters}</Text></View> : null}
          {showExample ? <View className="rounded-2xl bg-[#1D2029] p-4"><Text className="text-xs font-bold tracking-widest text-[#E8C98A]">EXEMPLE À ADAPTER</Text><Text className="mt-2 text-sm leading-6 text-[#E5DFD2]">{EXAMPLES[mission.id]}</Text></View> : null}
...
function AttemptPanel({ missionHint, request, result, notice, onRequest, onResult, onAnalyze }: { missionHint: string; request: string; result: string; notice: string | null; onRequest: (value: string) => void; onResult: (value: string) => void; onAnalyze: () => void }) {
  return <View className="gap-4 rounded-3xl border border-[#343947] bg-[#151820] p-5"><View><Text className="text-xs font-bold tracking-widest text-[#E8C98A]">1 · VOTRE TENTATIVE</Text><Text className="mt-2 text-lg font-bold text-[#F5F1E8]">Ne demandez pas la réponse parfaite.</Text><Text className="mt-1 text-sm leading-5 text-[#AEB4C0]">{missionHint}</Text></View><TextInput accessibilityLabel="Demande que vous avez faite à l’intelligence artificielle" value={request} onChangeText={onRequest} placeholder="Copiez ou écrivez votre demande…" placeholderTextColor="#788899" multiline style={styles.input} textAlignVertical="top" /><View><Text className="text-xs font-bold tracking-widest text-[#E8C98A]">2 · RÉSULTAT OBSERVÉ</Text><Text className="mt-2 text-sm leading-5 text-[#AEB4C0]">Qu’a répondu l’IA, et qu’est-ce qui vous semble faible ou incomplet ?</Text></View><TextInput accessibilityLabel="Résultat obtenu avec l’intelligence artificielle" value={result} onChangeText={onResult} placeholder="Décrivez le résultat réel…" placeholderTextColor="#788899" multiline style={styles.input} textAlignVertical="top" /><TouchableOpacity accessibilityRole="button" accessibilityLabel="Diagnostiquer ma tentative" onPress={onAnalyze} activeOpacity={0.8} className="items-center rounded-xl bg-[#D6B36A] px-4 py-4" disabled={false}><Text className="font-bold text-[#07080C]">Diagnostiquer ma tentative</Text></TouchableOpacity>{notice ? <Text accessibilityLiveRegion="polite" className="text-sm leading-5 text-[#E5DFD2]">{notice}</Text> : null}</View>;
}
...
function DiagnosisPanel({ missionId, diagnosis, learning, onRetry, onOpenCoach }: { missionId: MissionTrackId; diagnosis: MissionDiagnosis; learning: MissionLearningState; onRetry: () => void; onOpenCoach: () => void }) {
  ...
  return (
    <View className="gap-5 rounded-3xl border border-[#C89D56] bg-[#151820] p-5">
      <View className="flex-row items-start justify-between gap-4">
        <View className="flex-1"><Text className="text-xs font-bold tracking-widest text-[#E8C98A]">3 · DIAGNOSTIC PÉDAGOGIQUE</Text><Text className="mt-2 text-2xl font-bold leading-8 text-[#F5F1E8]">Voici ce qui fera progresser votre prochaine tentative.</Text></View>
        <View className="items-center rounded-2xl border border-[#C89D56] bg-[#071B3A] px-3 py-2"><Text className="text-2xl font-bold text-[#E8C98A]">{diagnosis.overallScore}</Text><Text className="text-[10px] font-bold tracking-wide text-[#AEB4C0]">/ 100</Text></View>
      </View>

      <TouchableOpacity accessibilityRole="button" accessibilityState={{ expanded: showScoreGuide }} accessibilityLabel="Comprendre le score de maîtrise" onPress={() => setShowScoreGuide((value) => !value)} activeOpacity={0.75} className="flex-row items-center justify-between rounded-xl bg-[#0B0D12] px-4 py-3"><Text className="text-sm font-semibold text-[#E5DFD2]">Comprendre le score de maîtrise</Text><Text className="text-lg text-[#E8C98A]">{showScoreGuide ? '−' : '+'}</Text></TouchableOpacity>
      {showScoreGuide ? <View className="rounded-2xl border border-[#343947] bg-[#1D2029] p-4"><Text className="text-sm leading-6 text-[#E5DFD2]">Le score mesure ce que votre tentative démontre aujourd’hui. Il ne récompense pas l’effort seul et il ne remplace pas vos XP.</Text></View> : null}
      ...
      <View className="gap-3"><View className="flex-row items-center justify-between"><Text className="text-base font-bold text-[#F5F1E8]">Vos capacités dans cette tentative</Text><Text className="text-xs text-[#AEB4C0]">Touchez pour explorer</Text></View><View className="flex-row gap-2"><CapabilityTile capabilityId="prompting" score={diagnosis.capabilityScores.prompting} selected={focusedCapability === 'prompting'} onPress={setFocusedCapability} /><CapabilityTile capabilityId="verification" score={diagnosis.capabilityScores.verification} selected={focusedCapability === 'verification'} onPress={setFocusedCapability} /><CapabilityTile capabilityId="reasoning" score={diagnosis.capabilityScores.reasoning} selected={focusedCapability === 'reasoning'} onPress={setFocusedCapability} /><CapabilityTile capabilityId="automation" score={diagnosis.capabilityScores.automation} selected={focusedCapability === 'automation'} onPress={setFocusedCapability} /></View></View>

      <View className="rounded-2xl border border-[#34557F] bg-[#0D1C31] p-4"><Text className="text-xs font-bold tracking-widest text-[#E8C98A]">FOCUS · {focusedDetails.label.toUpperCase()} · {focusedScore}/100</Text><Text className="mt-2 text-sm leading-6 text-[#E5DFD2]">{focusedDetails.action}</Text><Text className="mt-2 text-xs leading-5 text-[#8F96A4]">Votre niveau cumulé dans cette capacité : {learning.mastery[focusedCapability].score}/100.</Text></View>

      <TouchableOpacity accessibilityRole="button" accessibilityState={{ expanded: showCorrection }} accessibilityLabel="Voir la correction détaillée de l’intelligence artificielle" onPress={() => setShowCorrection((value) => !value)} activeOpacity={0.75} className="flex-row items-center justify-between rounded-xl border border-[#343947] bg-[#0B0D12] px-4 py-4"><View><Text className="font-bold text-[#F5F1E8]">Voir la correction de l’IA</Text><Text className="mt-0.5 text-xs text-[#AEB4C0]">Appliquez-la plutôt que de demander une réponse neuve.</Text></View><Text className="text-lg text-[#E8C98A]">{showCorrection ? '⌃' : '⌄'}</Text></TouchableOpacity>
      {showCorrection ? <View className="rounded-2xl border border-[#C89D56] bg-[#071B3A] p-4"><Text className="text-xs font-bold tracking-widest text-[#E8C98A]">CORRECTION À APPLIQUER</Text><Text className="mt-2 text-sm leading-6 text-[#E0EDFF]">{diagnosis.correction}</Text></View> : null}
      ...
      <View className="rounded-2xl bg-[#1D2029] p-4"><Text className="text-xs font-bold tracking-widest text-[#B5C7DD]">4 · PLAN DE RETENTATIVE</Text><Text className="mt-2 text-sm leading-6 text-[#F5F1E8]">{diagnosis.retryPrompt}</Text></View>
      <TouchableOpacity accessibilityRole="button" accessibilityLabel="Appliquer les conseils et faire une nouvelle tentative" onPress={onRetry} activeOpacity={0.8} className="items-center rounded-xl bg-[#D6B36A] px-4 py-4"><Text className="font-bold text-[#07080C]">Appliquer les conseils et réessayer</Text></TouchableOpacity>
      <TouchableOpacity accessibilityRole="button" accessibilityLabel="Ouvrir le coach IA pour approfondir la correction" onPress={onOpenCoach} activeOpacity={0.75} className="items-center rounded-xl border border-[#343947] bg-[#0B0D12] px-4 py-4"><Text className="font-semibold text-[#F5F1E8]">Approfondir avec le coach IA</Text></TouchableOpacity>
    </View>
  );
}
...
function CapabilityTile({ capabilityId, score, selected, onPress }: { capabilityId: MasteryCapabilityId; score: number; selected: boolean; onPress: (capabilityId: MasteryCapabilityId) => void }) {
  ...
  return <TouchableOpacity accessibilityRole="tab" accessibilityState={{ selected }} accessibilityLabel={`${details.label}, score ${score} sur 100`} onPress={() => onPress(capabilityId)} activeOpacity={0.75} className={selected ? 'flex-1 rounded-xl border border-[#C89D56] bg-[#17355C] px-2 py-3' : 'flex-1 rounded-xl border border-[#343947] bg-[#0B0D12] px-2 py-3'}><Text className="text-center text-base text-[#E8C98A]">{details.icon}</Text><Text className="mt-1 text-center text-[10px] font-semibold text-[#E5DFD2]" numberOfLines={1}>{details.label}</Text><Text className="mt-1 text-center text-sm font-bold text-[#F5F1E8]">{score}</Text></TouchableOpacity>;
}

const styles = StyleSheet.create({
  content: { paddingBottom: 36 },
  input: { minHeight: 116, borderWidth: 1, borderColor: '#343947', borderRadius: 14, backgroundColor: '#0B0D12', color: '#F5F1E8', fontSize: 16, lineHeight: 22, padding: 14 },
});
